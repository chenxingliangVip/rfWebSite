        ...
        methods: {
           ...
           upDateTable(pageNum,pageSize){//分页刷新表格
            ...
            cellStyle({ row, column, rowIndex, columnIndex }) {
                ...
                if ( name == "tableNum7") { //合同——阶段
                    if (row.tableNum7 == '待审核') {
                        color.color = "#e82626";
                    }else if(row.tableNum7 == '待增') {
                        color.color = "#eac11b";
                    }else if(row.tableNum7 == '进行中') {
                        color.color = "#3FC4B1";
                    }
                }
                if ( name == "tableData9") { //采购———采购任务——状态
                    if (row.tableData9 == '待审核') {
                        color.color = "#e82626";
                    }else if(row.tableData9 == '执行中') {
                        color.color = "#3FC4B1";
                    }
                }
                if ( name == "intableNum7" ) { //出入库——入库
                    if (row.intableNum7 == 0) {
                        color.color = "#606266";
                    }else {
                        color.color = "#f44336";
                    }
                }
                if ( name == "outtableNum6" ) { //出入库——出库
                    if (row.outtableNum6 == 0) {
                        color.color = "#606266";
                    }else {
                        color.color = "#f44336";
                    }
